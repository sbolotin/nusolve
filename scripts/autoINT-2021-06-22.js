...
function makeSetup()
{
    ...
    //
    setSigma2add(5.0e-9); // 15ns, since we start with SBD
    //
...
function try2resolveAmbiguities()
{
    ...
    config.activeBandIdx = bands.length - 1;
    session.process();
...
function printInfo()
{
    ...
    print(' -- printInfo(): Numbers                : ' + primaryBand.numTotal + '/' + primaryBand.numUsable + '/' + primaryBand.numProcessed +
                                                     '  ' + (primaryBand.wrms*1.0e12).toFixed(2));
...
/*  for debug:
    ...
    setup.path2SpoolFileOutput = pwd + '/Spools';
    setup.path2ReportOutput    = pwd + '/Reports';
