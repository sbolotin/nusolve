...
const   thresholdUT1adjustment          =3000; // microsec, == 3ms
const   thresholdUT1sigma               = 150; // microsec
const   thresholdSessionLength          =  18; // if session shorter than 18 minutes, do it yourself

//
//
//
//
const   selfName = 'autoINT';
var     hasShortBln;
...
//
const stnKey2estimate = Array('NYALE13S', 'SESHAN13');
...
function makeSetup()
{
    ...
    logger.write(Log.Inf, Log.Preproc, selfName + ': makeSetup(): end.');
    logger.write(Log.Inf, Log.Preproc, '');
};
...
function try2resolveAmbiguities()
{
    ...
    //
    logger.write(Log.Inf, Log.Preproc, '');
    logger.write(Log.Inf, Log.Preproc, selfName + ': try2resolveAmbiguities(): begin.');
    ...
    //
    // initially, remove everything that is outp of 5*sigma, no thresholds for absolute values:
    logger.write(Log.Inf, Log.Preproc, selfName + ': try2resolveAmbiguities(): running eliminateLargeOutliers:');
    session.eliminateLargeOutliers(config.activeBandIdx, maxNumOfPasses, 1.8);
    logger.write(Log.Inf, Log.Preproc, selfName + ': try2resolveAmbiguities(): running eliminateOutliersSimpleMode:');
    session.eliminateOutliersSimpleMode(config.activeBandIdx, maxNumOfPasses, 5.0, -1.0);
//->    logger.write(Log.Inf, Log.Preproc, selfName + ': try2resolveAmbiguities(): running eliminateOutliersSimpleMode (2):');
//->    session.eliminateOutliersSimpleMode(config.activeBandIdx, maxNumOfPasses, 4.0, -1.0);
...
//->    session.eliminateOutliersSimpleMode(config.activeBandIdx, maxNumOfPasses, 3.5, 1000.0e-9);
        session.eliminateOutliersSimpleMode(config.activeBandIdx, maxNumOfPasses, 5, -1.0);
        session.process();
        session.scanAmbiguityMultipliers(i);
        session.process();
...
//            session.eliminateOutliersSimpleMode(config.activeBandIdx, maxNumOfPasses, 3.0,  bands[i].getGroupDelaysAmbigSpacing()/2.0);
        ...
        logger.write(Log.Inf, Log.Preproc, selfName + ': try2resolveAmbiguities(): Calling eliminateLargeOutliers():');
        session.eliminateLargeOutliers(config.activeBandIdx, maxNumOfPasses, 1.4);

        // remove 5*sigma or if absolute value are bigger than 0.4 of typical ambig spacing:
        logger.write(Log.Inf, Log.Preproc, selfName + ': try2resolveAmbiguities(): Calling eliminateOutliersSimpleMode():');
        session.eliminateOutliersSimpleMode(i, maxNumOfPasses, 5, bands[i].groupDelaysAmbigSpacing*0.4);
    ...
    //
    // if stations more than 2, check for presence of new stations, found turn on estimation of their coordinates:
    if (2 < session.baselines.length)
...
//        session.eliminateOutliersSimpleMode(i, maxNumOfPasses, 3.0, bands[i].groupDelaysAmbigSpacing*0.25);
    ...
    logger.write(Log.Inf, Log.Preproc, '');
    logger.write(Log.Inf, Log.Preproc, selfName + ': try2resolveAmbiguities(): end.');
};
...
{
    logger.write(Log.Inf, Log.Preproc, '');
    logger.write(Log.Inf, Log.Preproc, selfName + ': checkCableCals(): begin.');
...
//      print(' .. station ' + stations[i].key + ' => ' + wrmsByStation[stations[i].key]*1.0e12  + ' ps wrms');
    ...
    logger.write(Log.Inf, Log.Preproc, selfName + ': checkCableCals(): end.');
    logger.write(Log.Inf, Log.Preproc, '');
};
...
{
    logger.write(Log.Inf, Log.Preproc, '');
    logger.write(Log.Inf, Log.Preproc, selfName + ': doReweightAndOutliers(): begin.');
    //
...
//  parsDescript.setMode4Parameter(Parameters.Bl_Clk,   Parameters.EstimateLocal);
    ...
    {
        reasonOfFailure.push('Too many potentially good observations are deselected: ' +  (primaryBand.numUsable - primaryBand.numProcessed) + ' of ' + primaryBand.numUsable);
        isAutoGood = false;
    };
    // tooo large wrms:
    ...
    logger.write(Log.Inf, Log.Preproc, selfName + ': doReweightAndOutliers(): end.');
    logger.write(Log.Inf, Log.Preproc, '');
};
...
//      setup.have2KeepSpoolFileReports = true;
    ...
    {
        logger.write(Log.Err, Log.Preproc, selfName + ': reading session "' + handler.fileName + '" has failed');
        logger.returnCode = 1;
        return logger.returnCode;
    };
};
