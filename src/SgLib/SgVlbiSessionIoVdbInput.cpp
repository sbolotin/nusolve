  ...
  const QStringList& masterfileSuffixes)
{
  ...
  QMap<QString, SgMatrix* >     residFringeAmplByChanByBand, residFringePhseByChanByBand,
                                ...
                                loFreqs_1ByBand, loFreqs_2ByBand, channelBandwidthByBand;
  // eoci
  
  QList<QString>                listOfBands;
  bool                          isVgosData;
  bool                          isNuSolveRunning;
  

  isVgosData = false;
  isNuSolveRunning = driverVersion->getSoftwareName().contains("nuSolve");
...
//  dbeditVersionByBand             .insert(bandKey, QVector< QVector<int> >());
    ...
    channelBandwidthByBand          .insert(bandKey, NULL);
  };
...
//                             &dbeditVersion=dbeditVersionByBand[bandKey];
    ...
    const SgMatrix             *residFringeAmplByChan=residFringeAmplByChanByBand[bandKey],
                               ...
                               *channelBandwidth=channelBandwidthByBand[bandKey];
    ...
    for (int obsIdx=0; obsIdx<numOfObs; obsIdx++)
    {
         ...
         )
      {
        ...
        if (singleBandDelays)
        {
          ...
          if (o->sbDelay().getSigma() < 5.0e-14)
          {
            o->owner()->addAttr(SgVlbiObservation::Attr_SBD_NOT_VALID);
            o->sbDelay().addAttr(SgVlbiMeasurement::Attr_NOT_VALID);
          };
        };
        // group delay:
        if (groupDelays)
        {
          ...
          if (o->grDelay().getSigma() < 1.0e-18)
          {
            //o->owner()->addAttr(SgVlbiObservation::Attr_NOT_VALID);
            o->grDelay().addAttr(SgVlbiMeasurement::Attr_NOT_VALID);
          };
        };
        // delay rate:
        if (rates)
        ...
        if (nOfC>0)
        {
          ...
          for (int j=0; j<nOfC; j++)
          {
            ...
            if (channelBandwidth)
              o->channelBandwidth()->setElement(j, channelBandwidth->getElement(obsIdx, j));
            else
              o->channelBandwidth()->setElement(j, o->getSampleRate()/2.0/1.0e6);
            ...
            if (ifId.size() && ifId!=QString(" ") && ifId!=QString("-"))
            {
...
//            cppsSoft_ != CPPS_PIMA &&
              vgosDb->getCorrelatorType() != SgVgosDb::CT_S2)
          {
            ...
            evaluateEffectiveFreqs(*o->numOfAccPeriodsByChan_USB(), *o->numOfAccPeriodsByChan_LSB(),
              ...
              *o->channelBandwidth(), o->getReferenceFrequency(), 
              nOfC, effFreq4GR, effFreq4PH, effFreq4RT, correlatorType, o);
            ...
            if (true)
            {
              evaluateEffectiveFreqs(*o->numOfAccPeriodsByChan_USB(), *o->numOfAccPeriodsByChan_LSB(),
                ...
                *o->numOfSamplesByChan_USB(), *o->numOfSamplesByChan_LSB(),
                *o->channelBandwidth(), o->getReferenceFrequency(), 
                nOfC, effFreq4GR, effFreq4PH, effFreq4RT, correlatorType, o, false, true);
              o->setTest (effFreq4GR);
              o->setTest2(effFreq4GR - o->grDelay().getEffFreq());
            };
            ...
            evaluateEffectiveFreqs(*o->numOfAccPeriodsByChan_USB(), *o->numOfAccPeriodsByChan_LSB(),
              ...
              *o->numOfSamplesByChan_USB(), *o->numOfSamplesByChan_LSB(),
              *o->channelBandwidth(), o->getReferenceFrequency(),
              nOfC, effFreq4GR, effFreq4PH, effFreq4RT, correlatorType, o, true);
        ...
        {
          //if (o->grDelay().getUnweightFlag()==1 || o->grDelay().getUnweightFlag()==2)
          if (o->grDelay().getUnweightFlag() != 0)
            o->grDelay().addAttr(SgVlbiMeasurement::Attr_NOT_VALID);
          if (o->phDelay().getUnweightFlag() != 0)
            o->phDelay().addAttr(SgVlbiMeasurement::Attr_NOT_VALID);
          if (o->phDRate().getUnweightFlag() != 0)
            o->phDRate().addAttr(SgVlbiMeasurement::Attr_NOT_VALID);
...
/*
        QString                 str("");
        if (dataFlag.size() <= 0)
          str += "missed data flags; ";
        if (0<dataFlag.size() && dataFlag.at(obsIdx) == -1)
          str += "data flag is -1; ";
        if (!ambigs)
          str += "no ambigs; ";
        if (ambigs && ambigs->getElement(obsIdx)<=0.0)
          str += "ambigs<0; ";
        if (ambigs && 0.0<ambigs->getElement(obsIdx) && !groupDelays)
          str += "missing groupDelays; ";
        if (2 < str.size())
          str.chop(2);
*/
        logger->write(SgLogger::DBG, SgLogger::IO_NCDF, className() +
          "::getDataFromVgosDb(): skipped observable #" + QString("").setNum(obsIdx) + 
          " at the " + bandKey + "-band: missed data"); //. reason: " + str);
      }
    };
    //
...
*/
    ...
    // correct the channell info:
    if (isNuSolveRunning /*&& !isVgosData*/)
    {
      // channel bandwidth:
      if (!channelBandwidth)
      {
        band->addAttr(SgVlbiBand::Attr_CHANNEL_INFO_MODIFIED);
        addAttr(Attr_FF_CHANNEL_INFO_MODIFIED);
        logger->write(SgLogger::DBG, SgLogger::IO_NCDF, className() + 
          "::getDataFromVgosDb(): channel bandwidth was set to the defaul value");
      };
      //
      // sideband order:
      if (getSidebandOrder() == SO_UNDEF)
      {
        // need to save a new version of the ChannelInfo.nc:
        if (!vgosDb->hasSidebandOrder())
        {
          band->addAttr(SgVlbiBand::Attr_CHANNEL_INFO_MODIFIED);
          addAttr(Attr_FF_CHANNEL_INFO_MODIFIED);
          logger->write(SgLogger::DBG, SgLogger::IO_NCDF, className() + 
            "::getDataFromVgosDb(): no sideband order found in the database, using its default value");
        };
        //
        setSidebandOrder(sbOrder);
        logger->write(SgLogger::DBG, SgLogger::IO_NCDF, className() + 
          "::getDataFromVgosDb(): the sideband order was set to " + sidebandOrderByType[sbOrder]);
      };
    };
    //
    // quick'n'dirty:
...
//  dbeditVersionByBand[bandKey].clear();
    ...
    if (channelBandwidthByBand[bandKey])
      delete channelBandwidthByBand[bandKey];
